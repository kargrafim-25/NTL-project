...
export default function AILoadingAnimation({ 
  ...
  return (
    <div className="flex flex-col items-center space-y-4 py-8" data-testid="ai-loading-animation">
        ...
        <div className="w-16 h-16 relative">
          <div className="absolute inset-0 rounded-full border-2 border-primary/30 animate-spin-slow">
            ...
            <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2">
              <div className="w-1 h-1 bg-secondary rounded-full animate-pulse delay-500"></div>
            </div>
            <div className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1/2">
              <div className="w-1 h-1 bg-primary rounded-full animate-pulse delay-1000"></div>
            </div>
            <div className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-1/2">
              <div className="w-1 h-1 bg-secondary rounded-full animate-pulse delay-1500"></div>
            </div>
          </div>
        </div>

        {/* Middle pulse ring */}
        <div className="absolute inset-2 rounded-full border border-secondary/50 animate-ping"></div>
        ...
        <div className="absolute inset-0 flex items-center justify-between opacity-60">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="relative">
              ...
              <div 
                className="w-px bg-gradient-to-b from-primary via-secondary to-primary animate-pulse"
                style={{
                  height: '20px',
              ...
              {/* Moving data points */}
              <div 
                className="absolute top-0 w-1 h-1 bg-primary rounded-full animate-bounce"
                style={{
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 128 32">
          ...
          <defs>
            <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              <stop offset="50%" stopColor="hsl(262, 69%, 58%)" stopOpacity="0.8" />
              <stop offset="100%" stopColor="hsl(221, 83%, 53%)" stopOpacity="0.3" />
            </linearGradient>
          </defs>
        </svg>
      </div>
      ...
      <div className="w-48 h-1 bg-muted/30 rounded-full overflow-hidden">
        ...
        <div className="h-full w-1/3 bg-gradient-to-r from-primary to-secondary animate-slide-right"></div>
      </div>
    </div>
  );
}